...
Para casos en los que la variable dependiente es binaria o representa conteos se utilizan los Modelos Lineales Generalizados (GLM), que permiten trabajar con distintos tipos de variables dependientes y distribuciones, como la binomial, Poisson o gamma.
...
Puesto que nuestra variable objetivo se ve así:
```{r}
#Histograma(Para ver la forma de la distribución):
...
Ajustamos un modelo de regresión logística para predecir si una publicación será popular (más de 275 interacciones) usando like, share y comment como variables predictoras.
...
Como vemos, podemos anticiparnos a los resultados ya que la varianza es mayor que la media.
...
Hemos ajustado el modelo de regresión binomial negativa para corregir la sobredispersión previamente detectada en el modelo de Poisson. 
...
En muchas ocasiones, las relaciones entre las variables son no lineales y complejas, lo que limita la efectividad de los modelos de regresión tradicionales como la regresión lineal o polinomial.

Los Modelos de Regresión Aditiva Generalizada (GAMs) ofrecen una solución poderosa y flexible para este problema, ya que permiten modelar relaciones no lineales sin necesidad de especificar de antemano la forma exacta de la no linealidad. En los GAMs cada predictor puede tener una forma funcional diferente, permitiendo capturar curvas, patrones no lineales y efectos complejos en los datos.


Las funciones de suavizado permiten modelar relaciones no lineales de manera flexible y controlada, evitando el sobreajuste. En los GAMs, las funciones de suavizado se implementan comúnmente mediante splines, que son funciones polinómicas definidas por tramos.

Vamos a ajustar un GAM con Splines
```{r}
library(mgcv)

 # Sustituimos espacios por puntos en los nombres de las variables
train_clean2 <- train_clean
names(train_clean2) <- make.names(names(train_clean2))  

# Ajuste del modelo GAM usando splines cúbicos con la variable 'like'
modelo_gam <- gam(Total.Interactions ~ s(like), data = train_clean2, method = "REML")

# Resumen del modelo
summary(modelo_gam)
```
Interpretamos los resultados obtenidos con summary. 

En primer lugar, el **edf (effective degrees of freedom)** indica el grado de flexibilidad del suavizado. Un edf cercano a 1 sugiere una relación lineal. Un edf mayor que 1 indica una relación no lineal. En este caso, tenemos un edf = 8.459, lo que indica que la relación entre las variables es no lineal, ya que es mayor que 1. 

El **p-valor** indica  si la función suavizada para este predictor es significativa. Tenemos que el p-valor < 2e-16. Como p-valor < 0.05, podemos rechazar la hipótiesis nula y concluir que la relación entre el predictor (like) y la variable dependiente (Total interactions) es estadísticamente significativa. Esto tiene sentido ya que 'Total Interactions' es la suma de like, share y comment.


Evaluación del ajuste de un GAM
```{r}
# Verificar la deviance explicada y el AIC
cat("Deviance explained:", summary(modelo_gam)$dev.expl, "\n")
cat("AIC del Modelo:", AIC(modelo_gam), "\n")
```

El **Deviance explained** indica el porcentaje de la variabilidad de los datos explicada por el modelo. En este caso es de 0.9952203, lo que sugiere que el modelo captura muy bien la variabilidad de los datos.

El **AIC** evalua la calidad del ajuste. Un AIC más bajo sugiere un modelo mejor ajustado. En este modelo el AIC es de 2099.987, no es muy bajo.


Veamos gráficamente la función suavizada.
```{r}
...
El **QQ Plot** compara los residuos del modelo con una distribución normal. La línea roja es lo que se esperaría bajo normalidad y lo negro son los residuos del modelo. Vemos que en los residuos hay una curvatura, lo que indica que nos residuos no son normales.

En el gráfico de **resids vs linear pred**, vemos que hay cierta forma de embudo, los residuos son más dispersos a medida que aumenta la predicción. Esto indica posible heterocedasticidad. 

El **histograma** nos ayuda a ver cómo de distribuyen los residuos. Refuerza lo visto en el Q-Q plot, no hay una normalidad perfecta.

Por último, el gráfico **response vs Fitted Values** muestra si el modelo está capturando la tendencia general de la respuesta. Buscamos una diagonal. Vemos que los puntos se alinean, pero hay mucha dispersión a medida que suben los valores ajustados. Esto implica que el modelo tiene mejor ajuste en el rango bajo de 'Total Interactions', pero le cuesta con los valores altos.



La validación cruzada es una técnica robusta para seleccionar el grado de suavizado. Como hemos visto anteriormente, en este enfoque, el conjunto de datos se divide en varios subconjuntos (folds), y el modelo se entrena y evalúa en diferentes combinaciones de estos subconjuntos.
```{r}
library(mgcv)
library(cvTools)

# Asignar variables con nuevos nombres
x1 <- train_clean2$like
y1 <- train_clean2$Total.Interactions
n1 <- length(y1)

# Configuración de validación cruzada 5-fold
set.seed(123)
folds <- cvFolds(n = n1, K = 5)
...
for (i in 1:5) {
  ...
  train_idx <- setdiff(1:n1, test_idx)
  
  modelo_gam_cv <- gam(y1 ~ s(x1), data = data.frame(x1 = x1[train_idx], y1 = y1[train_idx]), method = "REML")
  predicciones <- predict(modelo_gam_cv, newdata = data.frame(x1 = x1[test_idx]))
  
  errores[i] <- sqrt(mean((y1[test_idx] - predicciones)^2))  # RMSE
}
...
```
Tras realizar una validación cruzada, hemos obtenido un **RMSE** de 24.47838 indica que, en promedio, el modelo se equivoca por alrededor de 24 interacciones cuando predice el número total de interacciones de una publicación. Es un error bastante bajo, lo que sugiere que es un buen modelo.


Como 'Total Interactions' es la suma de 'like' 'comment' y 'share', vamos a probar el modelo con otra de las variables, para ver si el modelo sigue siendo bueno.
```{r}
# Ajuste del modelo GAM usando splines cúbicos con la variable 'Lifetime Post reach by people who like your Page'
modelo_gam2 <- gam(Total.Interactions ~ s(Lifetime.Post.reach.by.people.who.like.your.Page), data = train_clean2, method = "REML")
head(train_clean2)
# Resumen del modelo
summary(modelo_gam2)
```
En este caso, el **edf** es 7.729, lo que indica que de nuevo la relación entre las variables es no lineal, ya que es mayor que 1.

En cuanto al **p-valor**, tenemos que el p-valor < 2e-16. Como p-valor < 0.05, rechazamos la hipótesis nula y podemos concluir que la relación entre el predictor y la variable dependiente es estadísticamente significativa, en este caso entre 'Total interactions' y 'Lifetime Post reach by people who like your Page'


Evaluación del ajuste de un GAM
```{r}
# Verificar la deviance explicada y el AIC
cat("Deviance explained:", summary(modelo_gam2)$dev.expl, "\n")
cat("AIC del Modelo:", AIC(modelo_gam2), "\n")
```
El **Deviance explained** es de 0.6092445, lo que sugiere que el modelo captura la variabilidad de los datos, pero no de una forma tan buena como en el caso anterior.

El **AIC** de este modelo es de 3186.723, que es bastante alto.


```{r}
# Visualización de la función suavizada
plot(modelo_gam2, main = "Ajuste GAM con Splines Cúbicos", shade = TRUE)
...
En el **QQ Plot** los puntos se desvían de la línea recta (línea roja), especialmente en las colas. Esto indica que los residuos no siguen una distribución normal.

En el gráfico de **resids vs linear pred** , al igual que antes vemos que hay cierta forma de embudo, la dispersión de los residuos aumenta con el valor del predictor lineal. Esto indica posible heterocedasticidad. 

El **histograma** nos ayuda a ver cómo de distribuyen los residuos. Aunque tiene cierta simetría, hay una cola hacia la derecha, lo que refuerza la idea de que la distribución no es normal. Esto coincide con lo observado en el Q-Q plot.

Por último, el gráfico **response vs Fitted Values** muestra si el modelo está capturando la tendencia general de la respuesta. La relación no sigue una línea clara ni una forma sistemática. Hay alta dispersión y evidencia de valores extremos (outliers)
...
# Extraer variables directamente del data frame
x <- train_clean2$Lifetime.Post.reach.by.people.who.like.your.Page
y <- train_clean2$Total.Interactions
n <- length(y)

# Configuración de la validación cruzada 5-fold
set.seed(123)# Ajuste del modelo GAM usando splines cúbicos con la variable 'like'
modelo_gam <- gam(Total.Interactions ~ s(like), data = train_clean2, method = "REML")
folds <- cvFolds(n = n, K = 5)

# Inicializar vector para errores
errores <- numeric(5)
...
for (i in 1:5) {
  ...
  errores[i] <- sqrt(mean((y[test_idx] - predicciones)^2))  # RMSE
}
...
La validación curzada nos ha dado un **RMSE** de 183.3781 indica que, en promedio, este modelo se equivoca por alrededor de 183 interacciones cuando predice el número total de interacciones de una publicación. La mayoría de observaciones tienen entre o y 260 interacciones totales, por lo que un error de 183 es bastante alto, lo que indica que no es un buen modelo.
...
#test_clean
modelo <- lm(`Total Interactions` ~ `like`, data = test_clean)

#modelo polinomico
modelo_polinomico <- lm(comment ~ poly(`Total Interactions`, 2), data = test)


#modelo logaritmico
test_clean <- test_clean[test_clean$`Total Interactions` > 0, ]
modelo_log <- lm(log(test_clean$`Total Interactions`) ~ like + comment + share, data = test_clean)


#modelo sqrt
modelo_sqrt <- lm(sqrt(test$`Total Interactions`) ~ like, data = test)

#modelo con iteraccion
# Crear nueva variable de interacción
test$lifetime_share_interaccion <- test$`Lifetime Post Total Impressions` * test$share

# Ajustar modelo con interacción
modelo_interaccion <- lm(test$`Total Interactions` ~ `Lifetime Post Total Impressions` + share + lifetime_share_interaccion, data = test)


#modelo regresion logistca

# Crear nueva variable binaria popular en una copia de test
test_bin <- test
test_bin$popular <- ifelse(test_bin$`Total Interactions` > 275, 1, 0)



modelo_logistico <- glm(popular ~ ., data = test_bin, family = binomial)
# modelo poisson
modelo_poisson <- glm(`Total Interactions` ~ ., 
                      family = poisson, 
                      data = test_clean)
...
names(test_clean2) <- make.names(names(test_clean2)) 
# Ajuste del modelo GAM usando splines cúbicos con la variable 'like'
modelo_gam <- gam(Total.Interactions ~ s(like), data = test_clean2, method = "REML")

#modelo_gam

# Ajuste del modelo GAM usando splines cúbicos con la variable 'Lifetime Post reach by people who like your Page'
modelo_gam2 <- gam(Total.Interactions ~ s(Lifetime.Post.reach.by.people.who.like.your.Page), data = test_clean2, method = "REML")

```

En este análisis vamos a comparar distintos modelos de regresión (lineales, no lineales y generalizados) , tanto de la primera parte de la practica como de la segunda, utilizando varias métricas de evaluación para seleccionar el modelo más adecuado.Todo ello lo vamos a hacer con los datos de test. Para cada modelo, calcularemos las siguientes métricas (muchas de ellas ya usadas y explicadas previamente, pero las recordamos):

-**R² ajustado**: indica la proporción de la variabilidad explicada por el modelo, ajustada por el número de predictores; solo aplica a los modelos lineales.

-**AIC**: mide la calidad relativa del modelo penalizando la complejidad; un valor más bajo indica mejor ajuste.

-**BIC**: similar al AIC, pero penaliza más fuertemente la complejidad del modelo; también preferimos valores más bajos.

-**RMSE**: representa el error cuadrático medio de las predicciones, indicando en promedio qué tan lejos están las predicciones de los valores observados.

-**MAE**: mide el error absoluto medio entre las predicciones y los valores reales, sin penalizar tanto los errores grandes como lo hace el RMSE.

Estas métricas nos van a permitir evaluar y comparar el desempeño de modelos lineales simples, modelos polinómicos, splines, regresión logística, modelos de Poisson, binomial negativo, y modelos GAM. A partir de estos resultados seleccionaremos el modelo que logre un equilibrio entre buen ajuste y complejidad, para posteriormente validar con nuestros datos de validación.
...
cat("R² ajustado:", summary(modelo_polinomico)$adj.r.squared, "\n")
cat("AIC:", AIC(modelo_polinomico), "\n")
cat("BIC:", BIC(modelo_polinomico), "\n")
cat("RMSE:", rmse(modelo_polinomico), "\n")
cat("MAE:", mae(modelo_polinomico), "\n\n")
...
cat("R² ajustado:", summary(modelo_log)$adj.r.squared, "\n")
cat("AIC:", AIC(modelo_log), "\n")
cat("BIC:", BIC(modelo_log), "\n")
cat("RMSE:", rmse(modelo_log), "\n")
cat("MAE:", mae(modelo_log), "\n\n")
...
cat("R² ajustado:", summary(modelo_sqrt)$adj.r.squared, "\n")
cat("AIC:", AIC(modelo_sqrt), "\n")
cat("BIC:", BIC(modelo_sqrt), "\n")
cat("RMSE:", rmse(modelo_sqrt), "\n")
cat("MAE:", mae(modelo_sqrt), "\n\n")
...
cat("R² ajustado:", summary(modelo_interaccion)$adj.r.squared, "\n")
cat("AIC:", AIC(modelo_interaccion), "\n")
cat("BIC:", BIC(modelo_interaccion), "\n")
cat("RMSE:", rmse(modelo_interaccion), "\n")
cat("MAE:", mae(modelo_interaccion), "\n\n")

# ============================
# MODELO REGRESIÓN LOGÍSTICA
print("MODELO REGRESIÓN LOGÍSTICA")
cat("AIC:", AIC(modelo_logistico), "\n")
cat("BIC:", BIC(modelo_logistico), "\n")
cat("RMSE:", rmse(modelo_logistico), "\n")
cat("MAE:", mae(modelo_logistico), "\n\n")
...
cat("RMSE:", rmse(modelo_poisson), "\n")
cat("MAE:", mae(modelo_poisson), "\n\n")

# ============================
# MODELO GAM (splines cúbicos)
print("MODELO GAM (splines cúbicos con variable like)")
cat("AIC:", AIC(modelo_gam), "\n")
cat("BIC:", BIC(modelo_gam), "\n")
cat("RMSE:", rmse(modelo_gam), "\n")
cat("MAE:", mae(modelo_gam), "\n\n")

# ============================
# MODELO GAM2 (splines cúbicos )
print("MODELO GAM2 (splines cúbicos con la variable Lifetime Post reach by people who like your Page )")
cat("AIC:", AIC(modelo_gam2), "\n")
cat("BIC:", BIC(modelo_gam2), "\n")
cat("RMSE:", rmse(modelo_gam2), "\n")
cat("MAE:", mae(modelo_gam2), "\n\n")

```

A lo largo de este análisis hemos comparado diversos modelos de predicción con el objetivo de clasificar las publicaciones como populares o no populares, utilizando como variable dependiente una variable binaria creada a partir del umbral de 275 interacciones totales. Para ello hemos evaluado modelos lineales, polinómicos, transformaciones logarítmicas y de raíz cuadrada, splines, GAM, Poisson y regresión logística, aplicando métricas como el AIC, BIC, RMSE, MAE, R² ajustado y área bajo la curva (AUC).

Los resultados nos muestran que, aunque algunos modelos de regresión lineal tienen buenos valores de R² ajustado, presentan errores de predicción muy altos y peores valores de AIC y BIC. Por su parte, el modelo de regresión logística destaca al tener un AIC de 44 y un AUC de 0.9915, que indica una excelente capacidad discriminativa para diferenciar entre publicaciones populares y no populares. Además, sus errores RMSE y MAE don prácticamente nulos, confirmando su alto rendimiento en la clasificación.

Por tanto, considerando tanto la capacidad predictiva como la simplicidad e interpretabilidad, hemos decidido seleccionar el modelo de regresión logística como el modelo final, al ser el que mejor cumple con nuestro objetivo de clasificar las publicaciones en función de su popularidad de manera precisa y eficiente.


## Validación del modelo final 

Como hemos seleccionado el modelo de regresión logistica tenemos que incluir en nuestros datos de validación la variable popular que es uuna variable binaria que toma el valor 1 si Total Interactions es > 275, 0 , si es menor. Por lo tanto lo que vamos a predecir es si es popular o no. 

```{r}
#primero añadimos la variable binaria que hemos creado a validation
validation_bin <- na.omit(val)
validation_bin$popular <- ifelse(validation_bin$`Total Interactions` > 275, 1, 0)
...
```{r}
# 0. Crear la variable derivada también en validation_bin
validation_bin$lifetime_share_interaccion <- validation_bin$`Lifetime Post Total Impressions` * validation_bin$share
validation_bin$Type <- as.factor(validation_bin$Type)
validation_bin$Category <- as.factor(validation_bin$Category)

# 1. Predecir probabilidades
probabilidades <- predict(modelo_logistico, newdata = validation_bin, type = "response")
...
conf_matrix <- confusionMatrix(factor(predicciones_clase), factor(validation_bin$popular), positive = "1")
print(conf_matrix)

# 4. Curva ROC y AUC
roc_curve <- roc(validation_bin$popular, probabilidades)
plot(roc_curve, col = "blue", main = "Curva ROC - Validación modelo logístico")
auc_value <- auc(roc_curve)
cat("AUC:", auc_value, "\n")
conf_matrix$table

```
Tras realizar la validación del modelo de regresión logística sobre el conjunto de datos de validación que dividimos al principio de la práctica, observamos que el modelo mantiene un rendimiento demasiado bueno fuera de la muestra de entrenamiento. La matriz de confusión muestra que hemos clasificado correctamente 96 publicaciones no populares y 23 populares, con únicamente 2 falsos positivos y 2 falsos negativos, lo cual refleja una alta capacidad predictiva.

En cuánto a las métricas, obtenemos una accuracy global del 96.75%, lo que indica que el modelo acierta en casi todas las predicciones. Además, logramos una sensibilidad del 92%, lo que significa que somos capaces de identificar correctamente la mayoría de las publicaciones populares, y una especificidad del 97.96%, mostrando una gran habilidad para descartar correctamente las publicaciones no populares. El índice Kappa de 0.8996 refuerza esta idea al indicar un nivel de acuerdo muy alto más allá del azar.

Por otro lado, la curva ROC validada refleja visualmente esta excelente discriminación, y el valor de AUC = 0.9751 confirma lo que hemos mencionado antes, que  la capacidad del modelo para diferenciar entre publicaciones populares y no populares es muy alta. Este AUC, cercano a 1, es indicativo de un modelo muy sólido y fiable.

En conjunto, estos resultados nos permiten concluir que nuestro modelo de regresión logística no solo logra un alto rendimiento en los datos de entrenamiento, sino que también generaliza correctamente a nuevos datos, conservando una alta precisión, sensibilidad y especificidad. Por tanto, consideramos que este modelo es adecuado para predecir la popularidad de las publicaciones y puede implementarse con confianza en un entorno real para apoyar la toma de decisiones.
