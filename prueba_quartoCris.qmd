...
Al analizar los resultados del ajuste, hemos observado que ninguna de las variables predictoras resulta estadísticamente significativa, ya que todos los valores de p son iguales a 1. Esto indica que, según este modelo y los datos utilizados, no se puede afirmar que haya una relación significativa entre las variables independientes y la popularidad de una publicación.

Además, a pesar de que el modelo presenta una deviance residual muy baja y un AIC también bajo, lo cual en otros contextos podría sugerir un buen ajuste, estos resultados probablemente están influidos por un sobreajuste del modelo (overfitting) o problemas en la escala y codificación de las variables. Esto es especialmente evidente al observar los errores estándar extremadamente altos en los coeficientes, lo que podría estar indicando multicolinealidad o datos no adecuadamente normalizados.

En resumen, si bien el modelo lo hemos ajustado sin errores técnicos, los resultados no son confiables ni interpretables en términos estadísticos, y sería recomendable revisar la calidad de los datos, eliminar o transformar variables redundantes, y considerar métodos de regularización o modelos alternativos para mejorar la capacidad predictiva y la interpretación del modelo.
...
```
Tras aplicar un proceso de selección de variables mediante el método stepwise, el modelo de regresión logística fue reducido significativamente, conservando únicamente dos variables: Post Month y like. Con esta simplificación hemos buscado mejorar la eficiencia del modelo eliminando variables irrelevantes o redundantes.

No obstante, los resultados del modelo optimizado indican que ninguna de las variables seleccionadas resulta estadísticamente significativa (valores de p = 0.986). Además, los coeficientes estimados presentan errores estándar extremadamente altos, lo que nuevamente sugiere inestabilidad en el modelo y posibles problemas de escalamiento, multicolinealidad o escasa variabilidad en los datos.

A pesar de que el modelo muestra una reducción drástica en el AIC (6) y una residual deviance casi nula, estos indicadores pueden estar reflejando un sobreajuste y no necesariamente una mejora en la capacidad predictiva real. En este contexto, es importante no dejarse llevar únicamente por métricas como el AIC si el modelo carece de significancia estadística y estabilidad en los parámetros.
...
predicciones <- predict(modelo_cv, newdata = train_modelo_logistico, type = "prob")

table(predicciones[,1]>0.5,train_modelo_logistico$popular)

# Curva ROC
roc_curve <- roc(train_modelo_logistico$popular, predicciones$`1`)  # Clase "1" = popular
...
# Predecir las probabilidades para la clase positiva (por ejemplo, '1')
probabilidades <- predict(modelo_cv, type = "prob")

# Extraer las probabilidades de la clase '1'
probabilidades_positivas <- probabilidades[, 2]  # Cambia 2 por el índice de la clase positiva en tu modelo


# Crear la curva ROC
roc_curve <- roc(train_modelo_logistico$popular, probabilidades_positivas)
...
Tras realizar una validación cruzada de 3-fold sobre el modelo de regresión de Poisson, se obtuvo un RMSE (Root Mean Squared Error) de aproximadamente 1163.5, un R² de 0.602 y un MAE (Mean Absolute Error) de 184.4.
Estos resultados indican que el modelo tiene una capacidad de explicación moderada (explica aproximadamente el 60 % de la variabilidad en las interacciones totales) y un error medio aceptable considerando la escala de la variable objetivo.
Aunque el ajuste es razonable, existe margen de mejora si se quisiera optimizar el modelo.
