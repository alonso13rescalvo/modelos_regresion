...
```{r}
# Boxplot para ver la distribución de 'Total Interations' según la variable 'Paid'
library(ggplot2)
...
summary(modelo)
```
Onviamete nos da que el R^2 es muy alto ya que nuestro target consat de una suma de tres de las variables, entre ellas like.
...
R² = 0.8850409 significa que el 88.5% de la variabilidad en las interacciones es explicada por las variables incluidas en el modelo. Esto sugiere un modelo bastante fuerte, aunque no perfecto.

RMSE representa el error promedio en la prediccion. En este caso, RMSE = 71.69574 indica que la predicción del número total de interacciones tiene una desviación de aproximadamente 72 interacciones respecto a los valores reales.

MAE representa el error absoluto medio. Tenemos un MAE = 40.91904, lo que indica que el modelo se equivoca por aproximadamente 41 interacciones al predecir.
`
...
Variables seleccionadas (con coeficientes distintos de cero):
  ...
  + TypePhoto: -14.33 → Publicar fotos tiene un impacto negativo en la variable objetivo.
  + TypeStatus: -45.39 → Publicar solo un estado reduce considerablemente la respuesta esperada.
  + TypeVideo: +16.24 → Publicar videos tiene un impacto positivo significativo.
  + Category2 y Category3: +25.63 y +24.36 → Estas categorías específicas también contribuyen positivamente.
  + Paid: +8.92 → Realizar publicaciones pagadas mejora la respuesta esperada.
  + Lifetime Engaged Users: +1.16 → Incrementos en esta métrica aumentan la respuesta del modelo.
  + Lifetime Post Impressions by people who have liked your Page: +0.000578 → Pequeño impacto positivo.
  + Lifetime Post reach by people who like your Page: +0.000314 → Contribución positiva, aunque mínima.

  + Lifetime Post Total Reach y Lifetime Post Total Impressions: -0.0013 y +0.00019 respectivamente, mostrando ligeros efectos opuestos.

  + Post Month: -1.20 → Publicaciones en ciertos meses pueden tener un leve impacto negativo.
  
  Resumidamente, nos quedamos con: Type, Category, Paid, Lifetime Engaged Users, Lifetime Post Impressions by people who have liked your Page, Lifetime Post reach by people who like your Page y Post Month.

Variables como Post Weekday, Post Hour, Lifetime Post Consumptions, y Lifetime People who have liked your Page and engaged with your post han sido eliminadas por su baja relevancia predictiva según el modelo Lasso.
...
Se nota mucha variabilidad para valores bajos y medios de Total Interactions, lo que sugiere que un polinomio de segundo grado no es suficiente para capturar la relación real.
...
```
El modelo stepwise es el mejor: preciso, robusto y equilibrado.
Los modelos logarítmico y raíz cuadrada presentan mejor RMSE que el lineal, pero con menor R², por lo que son útiles si se prioriza estabilidad en valores bajos.
El modelo spline, aunque sea  flexible, no funciona bien en este nuestra caso.
