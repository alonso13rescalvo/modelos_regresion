...
# Otros mdelos de regresión
Los Modelos Aditivos Generalizados (GAMs) permiten capturar relaciones no lineales entre la variable dependiente y las variables independientes. 

Las funciones de suavizado permiten modelar relaciones no lineales de manera flexible y controlada, evitando el sobreajuste. En los GAMs, las funciones de suavizado se implementan comúnmente mediante splines.

Ajuste de un GAM con Splines
```{r}
library(mgcv)
...
En primer lugar, el edf indica el grado de flexibilidad del suavizado. s(like) tiene un edf = 8.459, lo que indica que la relación entre las variables es no lineal (ya que es mayor que 1). 
...
Un AIC más bajo sugiere un modelo mejor ajustado. En este modelo el AIC es de 2099.987, no es muy bajo.
...
El Q-Q Plot (arriba izquierda) compara los residuos del modelo con una distribución normal. La línea roja es lo que se esperaría bajo normalidad y lo negro son los residuos del modelo. Vemos que en los residuos hay una curvatura, lo que indica que nos residuos no son normales.

En el gráfico de resids vs linear pred (arriba derecha), vemos que hay cierta forma de embudo, los residuos son más dispersos a medida que aumenta la predicción. Esto indica posible heterocedasticidad. 

El histograma (abajo izquierda) nos ayuda a ver cómo de distribuyen los residuos. Refuerza lo visto en el Q-Q plot, no hay una normalidad perfecta.

Por último, el gráfico response vs Fitted Values (abajo derecha) muestra si el modelo está capturando la tendencia general de la respuesta. Buscamos una diagonal. Vemos que los puntos se alinean, pero hay mucha dispersión a medida que suben los valores ajustados. Esto implica que el modelo tiene mejor ajuste en el rango bajo de 'Total Interactions', pero le cuesta con los valores altos.
...
```
Un RMSE de 24.47838 indica que, en promedio, el modelo se equivoca por alrededor de 24 interacciones cuando predice el número total de interacciones de una publicación. Es un error bastante bajo, lo que sugiere que es un buen modelo.
...
En el Q-Q Plot (arriba izquierda) los puntos se desvían de la línea recta (línea roja), especialmente en las colas. Esto indica que los residuos no siguen una distribución normal.

En el gráfico de resids vs linear pred (arriba derecha), al igual que antes vemos que hay cierta forma de embudo, la dispersión de los residuos aumenta con el valor del predictor lineal. Esto indica posible heterocedasticidad. 

El histograma (abajo izquierda) nos ayuda a ver cómo de distribuyen los residuos. Aunque tiene cierta simetría, hay una cola hacia la derecha, lo que refuerza la idea de que la distribución no es normal. Esto coincide con lo observado en el Q-Q plot.

Por último, el gráfico response vs Fitted Values (abajo derecha) muestra si el modelo está capturando la tendencia general de la respuesta. La relación no sigue una línea clara ni una forma sistemática. Hay alta dispersión y evidencia de valores extremos (outliers)
...
Un RMSE de 183.3781 indica que, en promedio, este modelo se equivoca por alrededor de 183 interacciones cuando predice el número total de interacciones de una publicación. La mayoría de observaciones tienen entre o y 260 interacciones totales, por lo que un error de 183 es bastante alto, lo que indica que no es un buen modelo.
