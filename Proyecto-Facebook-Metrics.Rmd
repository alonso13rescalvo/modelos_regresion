...
# Primero, excluimos las variables "Total Interactions", "comment", "like" y "share"
...
```{r}
# Ajustar modelo Ridge, Lasso y Elastic Net y almacenar los coeficientes
coef_ridge <- coef(modelo_ridge_final)
coef_lasso <- coef(modelo_lasso_final)
coef_elastic <- coef(modelo_elastic_final)
coef_ols <- coef(modelo_lm)

# Convertir coeficientes a un formato de dataframe
coef_ridge_df <- as.data.frame(as.matrix(coef_ridge))
coef_lasso_df <- as.data.frame(as.matrix(coef_lasso))
coef_elastic_df <- as.data.frame(as.matrix(coef_elastic))
coef_ols_df <- as.data.frame(as.matrix(coef_ols))

# Asegurarse de que todos tengan las mismas columnas (variables)
# Añadir las columnas de coeficientes de los modelos faltantes si es necesario
# Asumimos que las variables de X1 son las mismas para todos los modelos

# Unificar las columnas de coeficientes con la misma cantidad de filas (variables)
all_coefs <- data.frame(
  RIDGE = coef_ridge_df[,1], 
  LASSO = coef_lasso_df[,1], 
  ELASTIC = coef_elastic_df[,1],
  OLS = coef_ols_df[,1]
)
# Asignar nombres a las columnas
colnames(all_coefs) <- c("RIDGE", "LASSO", "ELASTIC", "OLS")
